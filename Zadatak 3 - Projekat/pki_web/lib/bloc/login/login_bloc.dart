...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  Stream<LoginState> mapEventToState(LoginEvent event) async* {
    if (event is HomeLoginEvent) {
      Navigator.of(context).pushNamedAndRemoveUntil(RouteGenerator.MAIN_ROUTE, (Route<dynamic> route) => false);
    }
    if (event is GoToLoginLoginEvent) {
      yield InitialLoginState(enableErrorText);
    }
    if (event is GoToRegisterLoginEvent) {
      yield InitialLoginState(enableErrorText);
      Navigator.of(context).popAndPushNamed(RouteGenerator.REGISTER_ROUTE);
    }
    if (event is ProfileOptionsLoginEvent) {
      yield ProfileOptionsLoginState(enableErrorText);
    }
    if (event is InitialOptionsLoginEvent) {
      yield InitialLoginState(enableErrorText);
    }
